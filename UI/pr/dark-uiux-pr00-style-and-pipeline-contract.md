...
1. 不引入 atlas / region manifest schema；`VisualManifestEntry` 严格字段集合以 [README.md](./README.md#visual-manifest-field-policy) 为准，canonical / runtime / fixture / sample-pack manifest 禁止 `asciiGlyph` / `asciiColorHex`，仅允许 runtime loader 对历史 v1 输入做 decode-only 剥离。
