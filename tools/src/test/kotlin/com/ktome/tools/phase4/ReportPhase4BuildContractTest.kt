...
class ReportPhase4BuildContractTest {
    ...
    fun `phase4 aggregate helper keeps reportPhase4 artifact only semantics`() {
        ...
        assertFalse(buildScript.contains("finalizedBy(\"whiteBoxContentPack\")"))
        ...
        assertTrue(helperInvocation.contains("tasks.named(\"phase4LegacyReportOnly\")"))
        assertTrue(helperInvocation.contains("soloClearLabProducerTask"))
        assertTrue(helperInvocation.contains("phase4AggregateGameAliasProducers"))
        assertTrue(helperInvocation.contains("phase4AggregateClientAliasProducers"))
