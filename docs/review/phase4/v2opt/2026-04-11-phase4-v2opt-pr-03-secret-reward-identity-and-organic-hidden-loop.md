...
8. 若本 PR 需要新增或调整 hidden/reward 相关 aggregate/report task、兼容 alias 或 report-only 入口，必须复用统一 helper / declarative generation path；不得复制新的 `tasks.register<Test>` family。
