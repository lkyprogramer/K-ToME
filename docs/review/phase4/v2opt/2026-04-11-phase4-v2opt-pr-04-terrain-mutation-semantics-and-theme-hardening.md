...
2. 本 PR 的默认联合验收固定为：`terrainInteractionBatch`、`bossHarness`、`verifyOwner`、`phase4Report`。
...
6. 提交或审查本 PR 时，默认证据必须直接引用 canonical `report-phase4-summary.{json,md}` 与相关 owner artifact 中的段落或 metric：
   - `terrainInteractionEncounterRate.aggregate`
   - `terrainInteractionEncounterRate.per_zone_lower_bound`
   - `preferredTerrainTagsSeen` / `combatSampledZoneIds`
   - `bossHarness` 中可解释 `grey_crown` / theme 来源的字段
   不能只报命令 exit code，也不能只贴 legacy `phase4-summary` 快照。
7. 若本 PR 变更了 terrain/boss 相关 owner metric、threshold、note、owner 映射或 report schema，除跑显式 parity 外，还必须在同一提交同步更新：
...
8. 若本 PR 需要新增或调整 terrain/boss 相关 aggregate/report task、兼容 alias 或 report-only 入口，必须复用统一 helper / declarative generation path；不得复制新的 `tasks.register<Test>` family。
