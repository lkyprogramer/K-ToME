...
| `COMBAT_DECISION` frame | `PR-02` | `PR-05` | 已收口为单一 `CombatDecisionFrame`，内部 phase 为 `ACTION -> METHOD -> TARGET`；`Ctrl+S` 阻断并显示正式 toast | `InputHandlerTest`、`CombatDecisionFrameTest`、`CombatDecisionPanelTest`、`TileRendererCanvasTest` 与 `GoldenScreenshotHarnessTest` 覆盖真实 phase 机、面板消费和 golden 场景 | 旧 `phase4-uiux-pr02-combat-decision-stub-*` 不再允许新增；PR-05 使用 `phase4-uiux-pr05-*` telegraph/combat decision golden |
